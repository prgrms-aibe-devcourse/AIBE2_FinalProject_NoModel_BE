...
SELECT 
    ...
    -- 조회수 (사용횟수의 5~20배)
    CASE 
        WHEN ai_model_tb.is_public = b'1' AND ai_model_tb.price = 0.00 THEN FLOOR((1000 + (RAND() * 299000)) * (5 + RAND() * 15))
        WHEN ai_model_tb.is_public = b'1' AND ai_model_tb.price > 0.00 THEN FLOOR((100 + (RAND() * 49900)) * (5 + RAND() * 15))
        ELSE CASE WHEN ai_model_tb.is_public = b'1' THEN FLOOR((1 + (RAND() * 999)) * (2 + RAND() * 3)) ELSE 0 END
    END as view_count,
